...
- **所有变量、参数、返回值和闭包的参数等必须显式标注类型**，禁止依赖自动类型推导（如 `let x = Vec::new();` ❌ → `let x: Vec<T> = Vec::new();` ✅）
- 命名规则：
  - 变量名使用蛇形命名法
  ...
  - 宏名使用蛇形命名法
  - **语义化命名禁止缩写**
- 闭包参数需要显式标注类型：`|a: A| {}` 或 `|(a, b): (A, B)| {}`
- `format!` 宏：变量使用 `{变量}`（如 `format!("{a}")`），函数/方法返回值写在 `"{}"` 后面（如 `format!("{}", get_a())`）
...
// 本地crate/mod示例 (pub(crate))
pub(crate) use {a::aa, b::bb:{bbb, ccc}};

// 标准库crate/mod示例 (pub(crate))
pub(crate) use {a::aa, b::bb:{bbb, ccc}};

// 外部库crate/mod示例 (pub(crate))
pub(crate) use {a::aa, b::bb:{bbb, ccc}};

// 本地crate/mod示例 (pub(super))
pub(super) use {a::aa, b::bb:{bbb, ccc}};

// 标准库crate/mod示例 (pub(super))
pub(super) use {a::aa, b::bb:{bbb, ccc}};

// 外部库crate/mod示例 (pub(super))
pub(super) use {a::aa, b::bb:{bbb, ccc}};
...
- 严格遵循项目的：文件夹命名、模块划分方式、包导入风格、代码排序逻辑、编码约定
- **禁止函数体内出现空行**
...
- 测试代码遵循项目已有风格
- 单元测试放在项目根目录的 `tests` 里，子目录命名为对应 `src` 子目录，里面文件命名遵守上面的命名规则
