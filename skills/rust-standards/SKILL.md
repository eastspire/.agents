...
| [scripts/verify_lib_rs_doc_comment.py](scripts/verify_lib_rs_doc_comment.py) | **§2.4 lib.rs 必须 `//!` doc block 结构**(2026-09-26 第五轮 user 钦定,check 37):读最近 Cargo.toml 的 `[package].name`,校验 lib.rs 起头 3 行结构(`//! <pkg_name>` / `//!` 空行 / `//! <description>`)。`python3 <path>/verify_lib_rs_doc_comment.py <repo>` 单独跑;fixture compliant 0/exit 0,violating 2/exit 1。euv 实测 catch 5 真违规(包名 mismatch / 完全无 `//!` 块)。 |
