...
| `euv_navbar` / `euv_sidebar` / `euv_toc` / `euv_pagination` / `euv_dropdown` / `euv_drawer` | 站点组件(0.15 起,对齐 VuePress) | 见 euv-ui-standards §3.F | — |
| `euv_hero` / `euv_feature_grid` / `euv_markdown` / `euv_result` / `euv_doc_layout` | 站点组件(hero/特性栅格/md 渲染/状态页/文档三栏壳) | 见 euv-ui-standards §3.F | — |
| `euv_debug`                            | dev-only 响应式状态查看器:`value` 闭包每次 render 调用,内嵌 `Signal::get()` 即订阅 | — | — |

Component files live under `ui/src/component/<name>/` (`view/fn.rs` for the `#[component]` fn, `struct.rs` for the Props struct, `enum.rs` for any *Variant / *Color enums, plus `impl.rs`, `type.rs`, `const.rs` etc. as needed). 共 28 个带 `view/` 的组件目录(`ls -d ui/src/component/*/view`):`alert`, `badge`, `button`, `card`, `checkbox`, `debug`, `doc_layout`, `drawer`, `dropdown`, `field`, `header`, `hero`, `info`, `input`, `loading`, `logo`, `markdown`, `modal`, `nav`, `navbar`, `pagination`, `result`, `router`, `sidebar`, `tag`, `toc`, `vconsole`, `virtual_list`;另有纯 hook 工具组件(无 view/)`browser`, `camera`, `layout`, `theme`, `touch`。
...
13. **Mobile 顶部 safe-area 走 CSS 变量契约 `--euv-mobile-safe-top`,页面永不直接信任 `env()`**(PR #63,0.18.12 起)。letterbox 浏览器会谎报 env(VivoBrowser 类报 41px → "顶部大片空白"),沉浸式 WebView(euv-app)env 又是真值必须消费,静态规则无法区分两种宿主——所以 `c_mobile_header`/`c_mobile_nav_drawer` 消费 `var(--euv-mobile-safe-top, 0px)`(**默认 0**,浏览器永远贴顶),沉浸式宿主显式声明(`window.__EUV_IMMERSIVE__=true` 或 `<meta name="euv-immersive" content="true">`),框架 `UseEuvLayout::use_safe_area_fix` 检测到声明后实测 env 并写入 `<html>` 的该变量。`c_mobile_app_root` 的 `padding-top` 保持 `0px`(root 不吃 safe-area,避免双重间距)。euv-app 侧注入点 = `src-tauri/src/cache/fn.rs` `on_page_load` → `webview.eval`(euv-app PR #4)。
14. **headless 测试模拟 safe-area:直接设变量** `document.documentElement.style.setProperty("--euv-mobile-safe-top", "41px")`,header 高度应立即从 52px 变 93px。env() 在 headless Chrome 永远返回 0px(没有 notch、URL bar、status bar),所以框架实测路径(immersive 声明 → env → 变量)在 headless 只能验证"声明了但 env=0 → 保持 52px"这一支。`use_safe_area_fix` 仍会缓存 env 到 thread-local(`ui/src/component/layout/hook/static.rs`)供 `--safe-area-inset-*` 消费方使用。
15. **"顶部空白"类报告先分清两种成因再动手**。(a) 空白在状态栏区域 = safe-area padding 误消费(谎报 env 的浏览器)→ 由 #13 的变量契约根治;(b) header 与系统状态栏文字重合 = 沉浸式宿主没声明 → 宿主侧注入声明(euv-app PR #4 的做法)。历史上 min/max base+cap 公式(PR #54→#57)、raw env(#58)、完全删 env(#61)都是单方面修一边砸另一边,全部已被 #63 的变量契约取代,不要回退到那些方案。
