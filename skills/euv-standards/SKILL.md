---
name: euv-standards
description: '**euv 框架完整 API + 坑表 — 与 euv 框架打交道时必加载**。版本 0.13.3,edition 2024,monorepo 6 个工作区(crate `core` + `engine` + `macros` + `ui` + `cli` + `example`),根 crate 只是个 2 行 `pub use` 壳。覆盖:html! / class! / vars! / var! / #[component] / watch! / computed! 宏完整使用规范 + euv-core App / Signal<T> / HookContext / VirtualNode 全部公开 API + class! extends 拼接语义 + auto-get 单段标识符自动 .get() 语法糖 + 22 个 ui 组件(Button / Input / Checkbox / Radio / Select / Slider / Switch / Textarea / Form / Modal / Drawer / Tooltip / Popover / Toast / Tabs / Accordion / Table / Pagination / Menu / List / Card / Image)+ 6 个模块布局(core/engine/ui/macros/cli/example)+ 完整 example 项目子 crate 路由/页面/特有 class 清单 + wasm-pack 编译流程 + euv-cli 命令(euv build / euv fmt / euv server / euv mode)+ 与 euv-ui-standards 互锁(写页面/组件时同时加载)。触发关键词:euv、euv-core、euv-engine、euv-ui、euv-macros、euv-cli、Signal、html!、class!、computed!、watch!、vars!、var!、#[component]、VirtualNode、use_signal、App::mount、wasm-pack、euv-cli、auto_value、euv_component_registry_cache、HookContext、auto-get、mount_to_body、request_animation_frame、LifecyclePhase、use_effect、use_memo、use_callback、euv_macro_rules、to_vnode、diff_vnode、patch_vnode、create_dom、create_text_node、create_fragment、create_component、create_portal。**当且仅当任务完全不使用 euv**才不加载。'
license: MIT
---
