...
| 文档站/长页面切换主题后**一屏以下区域配色不对**（暗色模式下页面底部一大片白）                                                        | `c_app_root` 是 `height: 100%` 的视口锁定 app 壳（example 内部容器滚动）；文档站是 document 滚动，根元素只有 1 视口高，主题 `background` 到 1 视口高度就断了。**根 class 覆盖 `height: "auto"`（保留 `min-height: "100%"`)**；亮色模式白底看不出，暗色才暴露（verified 0.16.0, euv-docs PR #8)                                                                                                       |
